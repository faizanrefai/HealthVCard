...
-(void)JSONDel{
	...
	NSString *url=[NSString stringWithFormat:@"%@delete_import_address.php?aid=%@",objappdel.strUrl,objappdel.actID];
	NSMutableURLRequest *request=[NSMutableURLRequest requestWithURL:[NSURL URLWithString:url]];
	JSONParser *parser;
	parser= [[JSONParser alloc] initWithRequestForThread:request sel:@selector(searchResult11:) andHandler:self];
...
-(void)JSONSave{
	...
	if(objappdel.flag==13){
	url=[NSString stringWithFormat:@"%@update_import_address.php?aid=%@&Title=%@&Address=%@&State=%@&City=%@&Zip=%@&Country=%@&Phone_home=%@&Phone_work=%@&Phone_cell=%@&Email=%@&Additional_Info=%@",
				   objappdel.strUrl,objappdel.actID,txttitle1,txtadd1,txtstate1,txtcity1,txtpostal1,txtcountry1,txtphone1,txtwork1,txtcell1,txtemail1,txtview1];
	}
	else{
		
		url=[NSString stringWithFormat:@"%@add_Important_Address.php?Title=%@&Address=%@&State=%@&Zip=%@&Country=%@&City=%@&Phone_home=%@&Phone_work=%@&Phone_cell=%@&Email=%@&Additional_Info=%@&userid=%@&account_id=%@",
			objappdel.strUrl,txttitle1,txtadd1,txtstate1,txtpostal1,txtcountry1,txtcity1,txtphone1,txtwork1,txtcell1,txtemail1,txtview1,objappdel.appUserId,objappdel.strAccount_id];
...
-(void)JSONDisplay{
	...
	NSString *url=[NSString stringWithFormat:@"%@import_address_list.php?aid=%@",objappdel.strUrl,objappdel.actID];
	NSMutableURLRequest *request=[NSMutableURLRequest requestWithURL:[NSURL URLWithString:url]];
	JSONParser *parser;
...
-(IBAction)deletedata{
    ...
    [txtpostal resignFirstResponder];
    [txtstate resignFirstResponder];
    UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"Are You Sure" message:[NSString stringWithFormat:@"You are about to delete everything pertaining to Important Address %@",txttitle.text] delegate:self cancelButtonTitle:nil otherButtonTitles:@"Confirm",@"Cancel", nil];
	alert.tag=2;
	[alert show];
    [alert release];
