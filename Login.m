...
-(void)JSON{
	...
	NSString *url=[NSString stringWithFormat:@"%@login.php?LoginID=%@&Password=%@",objappdel.strUrl,txtName.text,txtPassword.text];
...
-(void)searchResult:(NSDictionary*)dictionary{
	...
	objappdel.appUserId=[[NSString alloc]init];
	...
	[AlertHandler showAlertForProcess];
	//start = [NSDate timeIntervalSinceReferenceDate];
	NSString *url=[NSString stringWithFormat:@"%@forgot_password.php?Email=%@",objappdel.strUrl,txtEmail.text];
	NSMutableURLRequest *request=[NSMutableURLRequest requestWithURL:[NSURL URLWithString:url]];
	JSONParser *parser;
	parser= [[JSONParser alloc] initWithRequestForThread:request sel:@selector(maildetail:) andHandler:self];
...
-(void)viewWillAppear:(BOOL)animated{
	txtName.text=@"BobJones";
	txtPassword.text=@"BobJones";
