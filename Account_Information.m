...
-(void)JSON{
	...
	NSString *url=[NSString stringWithFormat:@"%@account_Detalis_Accountid.php?account_id=%@",appdel.strUrl,appdel.strAccount_id];
...
-(void)JSONDelete{
	...
	NSString *url=[NSString stringWithFormat:@"%@delete_account.php?userid=%@&account_id=%@",appdel.strUrl,appdel.appUserId,appdel.actID];
	NSMutableURLRequest *request=[NSMutableURLRequest requestWithURL:[NSURL URLWithString:url]];
	JSONParser *parser;
...
-(void)JSONSave{
	...
	NSString *url=[NSString stringWithFormat:@"%@update_account.php?account_id=%@&name=%@&address=%@&city=%@&province=%@&postal_code=%@&DOB=%@&SIN=%@&health_card=%@&additional_info=%@",
				   appdel.strUrl,appdel.actID,txtName1,txtAdd1,txtCity1,txtProvince1,txtPostalCode1,txtDob1,txtSin1,txtHelthcardno1,txtAdiinfo1];
	NSMutableURLRequest *request=[NSMutableURLRequest requestWithURL:[NSURL URLWithString:url]];
	JSONParser *parser;
...
-(IBAction)Delete
{
    ...
    UIAlertView *alert=[[UIAlertView alloc]initWithTitle:[NSString stringWithFormat:@"You are about to delete all information associated with %@",appdel.appActName] message:@"This is final and not reversible.Please confirm you want to proceed with the deletion." delegate:self cancelButtonTitle:nil otherButtonTitles:@"Confirm",@"Cancel", nil];
    alert.tag=2;
	[alert show];
    [alert release];
